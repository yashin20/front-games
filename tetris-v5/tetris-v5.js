...
var currentShape; // 현재 블록
var score, level, levelStack=0; // 점수, 레벨, 레벨 업 카운트
var gemNum; //gem개수수

/*5블록 미리보기 배열*/
var nextShapes = []; //이후 5개 블록 저장 배열
...
function init() {
    ...
    shapeCell=[];
    shapePoint=[1,1];
    score = 0, level = 1; gemNum = 0;
    initializeNextShapes();
    createShape();
}
 ...
 */
//배열 초기화 - 5개 채우기
function initializeNextShapes() {
    while (nextShapes.length < 5) {
        var shapeIdx = chooseNextShape();
        nextShapes.push(shapeIdx);
    }
}
...
function createShape() {
    ...
    currentShape = nextShapes.shift(); //다음 블록 가져오기
    shapeColor = shapeColorMap[currentShape]; //색상 설정
...
function chooseNextShape() {
    ...
    // (Global var) lastShapeIndex : 이전 생성 Shape 저장장
...
function displayNextShapes() {
    initNextTable();
    for (var n = 0; n < 5; n++) {
        var shape = shapeArray[nextShapes[n]];
        var color = shapeColorMap[nextShapes[n]];
        for (var i = 0; i < shape.length; i++) {
            var y = shape[i][0]; // 각 블록을 다른 위치에 배치
            var x = shape[i][1];
            document.getElementById(String(y) + String(x) + "-" + String(n)).style.background = color;
        }
    }
}
...
function initNextTable() {
    ...
    for (var n = 0; n < 5; n++) {
      // 4X4
        for (var i = 0; i < 4; i++) {
            for (var j = 0; j < 4; j++) {
                document.getElementById(String(i) + String(j) + "-" + String(n)).style.background = "rgb(9, 17, 26)";
            }
        } 
...
function updateGemNum(){
    gemNum = parseInt(score / 500); 
}
function randomGem() {
    //img/gem1.png
    imgURL = "img/gem" + String(Math.floor(Math.random() * 9 + 1)) + ".png";
}
function displayGem() {
    if (gemNum > 0) {
        for (var i = 1; i <= gemNum; i++) {
            randomGem();
            document.getElementById("gem-" + String(i)).style.backgroundImage = "url('" + imgURL + "')";

            document.getElementById("gem-" + String(i)).style.backgroundSize = "cover";
            document.getElementById("gem-" + String(i)).style.backgroundPosition = "center";
            document.getElementById("gem-" + String(i)).style.backgroundRepeat = "no-repeat";
        }
    }
}
function initGemTable() {
    for (var i = 1; i <= 52; i++) {
        document.getElementById("gem-" + String(i)).style.background = "rgb(9, 17, 26)";
    }
}
...
"!LEVEL \n" +
"Line Clear : \'current level\' * 3 => NEXT LEVEL\n" +
"Gem : per 500";
 ...
 * Hold 기능 구현
 */
let holdShapeIdx = null;
let canHold = true;

function hold() {
    if (!canHold) return; //한턴에 한번만 허용

    clearTimeout(movingThread); // 기존 블록 이동을 취소
    removeShape(); //현재 블록을 화면에서 지운다.
    //블록 위치 삭제
    shapeCell = [];

    //currentShape가 회전되있는경우 -> 메인 Shape으로 변경
    currentShape = shapeMap[currentShape];

    if (holdShapeIdx == null) {
        //홀드된 블록이 없으면 현재 블록을 저장하고 새 조각 채우기 
        holdShapeIdx = currentShape;
        createShape();
    } else {
        //홀드된 블록이 있으면 현재 조각과 교체
        var temp = holdShapeIdx;
        holdShapeIdx = currentShape;
        currentShape = temp;
        spawnHoldShape();
    }
...
function spawnHoldShape() {
    //블록 중심 초기화
    shapePoint[0] = createPoint[0];
    shapePoint[1] = createPoint[1];

    shapeColor = shapeColorMap[currentShape];
...
function displayHoldShape() {
    ...
    var shape = shapeArray[holdShapeIdx];
    var color = shapeColorMap[holdShapeIdx];

    for (var i = 0; i < shape.length; i++) {
        var y = shape[i][0]; // 각 블록을 다른 위치에 배치
        var x = shape[i][1];
        document.getElementById("hold-" + String(y) + String(x)).style.background = color;
    }
}

// 다음 블록 표시 영역 초기화
function initHoldTable() {
    //4 X 4
    for (var i = 0; i < 4; i++) {
        for (var j = 0; j < 4; j++) {
            document.getElementById("hold-" + String(i) + String(j)).style.background = "rgb(9, 17, 26)";
        }
    }
}